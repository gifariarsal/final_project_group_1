...
const profileController = {
  patchChangeName: async (req, res) => {
    try {
      ...
      if (newName === findUser.username) {
        return res
          .status(500)
          .json({ message: "Please select other username" });
      }
      await db.sequelize.transaction(async (t) => {
        ...
        return res
          .status(200)
          .json({ message: "Name successfully change", data: findUser });
      });
    } catch (error) {
      return res
        .status(500)
        .json({ message: "Name already exist", error: error.message });
    }
  },
  patchChangeGender: async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ message: "Failed, Butuh Oplas", error: error.message });
    }
  },
  patchChangeBirthdate: async (req, res) => {
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        ...
        return res
          .status(200)
          .json({ message: "Success", data: result, find: findUser });
      });
    } catch (error) {
  ...
  patchChangePassword: async (req, res) => {
    try {
      ...
      await db.sequelize.transaction(async (t) => {
        ...
        return res
          .status(200)
          .json({ message: "Password Successfully Change", data: result });
      });
    } catch (error) {
      return res.status(500).json({ message: "Failed", error: error.message });
    }
  ...
  forgotPassword: async (req, res) => {
    try {
      ...
      const data = await fs.readFile(
        path.resolve(__dirname, "../emails/resetPassword.html"),
        "utf-8"
      );
